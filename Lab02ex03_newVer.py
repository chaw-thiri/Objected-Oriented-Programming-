...
// the final matrix would be the row of 1st matrix and col of the second matrix 
	for (int i = 0; i < row; ++i) { // row = row1
		for (int j = 0; j < col; ++j) { // col = col2 , reference:  Fact 1 
			int sum = 0; // sum would be a diffent value for every index of the matix, thus it lies after the row and col
